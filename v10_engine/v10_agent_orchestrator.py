...
你将获得由 V10 泊松数学引擎计算出的纯粹量化数据（基准面），以及 9 维雷达采集到的场外情报（伤停、天气、疲劳度、资金异动、主裁风格等）。

请遵循以下极度严格的金融风控原则撰写《V10 终极实盘决策内参》，并且**必须**完全采用以下五大版块的结构排版：
...
def run_orchestrator(date_str):
    print(f"🚀 [Orchestrator] Initiating V10.0 Pure Predictive Sequence for {date_str}...")
    ...
    mega_context = f"# V10.0 Agent Ultimate Context: {date_str}\n\n"
    ...
    for match_title, match_data in results_dict.items():
        ...
        mega_context += "### V10 Dynamic Quant Base Data (Post-Penalty)\n```json\n"
